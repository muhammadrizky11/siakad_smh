...
//nilai
$filenya = "lap_tgl.php";
$judul = "[PEMBINAAN]. Lap. Per Tanggal";
$judulku = "$judul";
$judulx = $judul;
...
if ($_POST['btnEX'])
	...
	for ($k=1;$k<=$days_in_month;$k++) 
		...
		//jumlah 
		$qku3 = mysqli_query($koneksi, "SELECT kd FROM siswa_pelanggaran ".
										"WHERE round(DATE_FORMAT(bina_tgl, '%d')) = '$k' ".
										"AND round(DATE_FORMAT(bina_tgl, '%m')) = '$ubln' ".
										"AND round(DATE_FORMAT(bina_tgl, '%Y')) = '$uthn'");
		...
		//rincian
		$qku31 = mysqli_query($koneksi, "SELECT * FROM siswa_pelanggaran ".
										"WHERE round(DATE_FORMAT(bina_tgl, '%d')) = '$k' ".
										"AND round(DATE_FORMAT(bina_tgl, '%m')) = '$ubln' ".
										"AND round(DATE_FORMAT(bina_tgl, '%Y')) = '$uthn'");
	...
	for ($k=1;$k<=$days_in_month;$k++) 
		...
		if (!empty($tku31))
			...
			do
				...
				$ku31_pnilai = balikin($rku31['point_nilai']);
				$ku31_pket = balikin($rku31['point_ket']);
				$ku31_bnama = balikin($rku31['bina_nama']);
				$ku31_bket = balikin($rku31['bina_ket']);
				...
				Point:<b>$ku31_pnilai</b>
				<hr>
				Pembinaan: <b>$ku31_bnama. $ku31_bket</b>
