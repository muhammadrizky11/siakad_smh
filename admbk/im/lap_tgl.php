...
//nilai
$filenya = "lap_tgl.php";
$judul = "[IJIN MASUK PULANG]. Lap. Per Tanggal";
$judulku = "$judul";
$judulx = $judul;
...
if ($_POST['btnEX'])
	...
	for ($k=1;$k<=$days_in_month;$k++) 
		...
		//jumlah 
		$qku3 = mysqli_query($koneksi, "SELECT kd FROM user_ijin ".
		...
		$rku3 = mysqli_fetch_assoc($qku3);
		$tku3_imasuk = mysqli_num_rows($qku3);
		...
		$rku3 = mysqli_fetch_assoc($qku3);
		$tku3_ipulang = mysqli_num_rows($qku3);
		...
		echo '<td>'.$k.'.</td>
		<td>
		<font color="red">'.$tku3.'</font>
		<br>
		IJIN MASUK:<b>'.$tku3_imasuk.'</b>
		<br>
		IJIN PULANG:<b>'.$tku3_ipulang.'</b>
		</td>
		<td>';

		//rincian
		$qku31 = mysqli_query($koneksi, "SELECT kd FROM user_ijin ".
										"WHERE round(DATE_FORMAT(postdate, '%d')) = '$k' ".
										"AND round(DATE_FORMAT(postdate, '%m')) = '$ubln' ".
										"AND round(DATE_FORMAT(postdate, '%Y')) = '$uthn' ".
		...
		$qku31 = mysqli_query($koneksi, "SELECT kd FROM user_ijin ".
										...
										"AND status = 'IJIN MASUK'");
		$rku31 = mysqli_fetch_assoc($qku31);
		$tku31_imasuk = mysqli_num_rows($qku31);
		...
		$qku31 = mysqli_query($koneksi, "SELECT kd FROM user_ijin ".
										...
										"AND status = 'IJIN PULANG'");
		$rku31 = mysqli_fetch_assoc($qku31);
		$tku31_ipulang = mysqli_num_rows($qku31);
		

		
		echo '<font color="red">'.$tku31.'</font>
		<br>
		IJIN MASUK:<b>'.$tku31_imasuk.'</b>
		<br>
		IJIN PULANG:<b>'.$tku31_ipulang.'</b>
		</td>
