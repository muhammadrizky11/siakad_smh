...
//nilai
$filenya = "m_tp.php";
$judul = "Data Tujuan Pembelajaran";
...
if ($_POST['btnSMP'])
	...
	mysqli_query($koneksi, "DELETE FROM kurmer_mapel_tp ".
								"WHERE tapel = '$tapelkd' ".
								"AND kelas = '$kelkd' ".
								"AND smt = '$smt' ".
								"AND kode = '$mkode'");	
	...
	for($i=1;$i<=25;$i++)
		...
		//query
		mysqli_query($koneksi, "INSERT INTO kurmer_mapel_tp(kd, tapel, kelas, smt, ".
									"kode, nama, tp_kode, tp_nama, postdate) VALUES ".
									"('$xyz', '$tapelkd', '$kelkd', '$smt', ".
									"'$mkode', '$mnama', '$i', '$nilku', '$today')");
		}
...
if ($_POST['btnEX'])
	...
	$sqlcount = "SELECT * FROM kurmer_mapel_tp ".
					"WHERE tapel = '$tapelkd' ".
					"AND kelas = '$kelkd' ".
					"AND smt = '$smt' ".
					"AND kode = '$mkode' ".
	...
	<h3>DAFTAR TUJUAN PEMBELAJARAN</h3>
	...
	for ($k=1;$k<=25;$k++)
		...
		//nilai
		$qku = mysqli_query($koneksi, "SELECT * FROM kurmer_mapel_tp ".
											"WHERE tapel = '$tapelkd' ".
											"AND kelas = '$kelkd' ".
											"AND smt = '$smt' ".
											"AND kode = '$mkode' ".
		...
		$i_pnilai = balikin($rku['tp_nama']);

		
									
		echo '<td align="left">'.$i_pnilai.'</td>';
	...
	if ($s == "import")
		...
		<tbody>';
			...
			for ($k=1;$k<=25;$k++)
				...
				mysqli_query($koneksi, "DELETE FROM kurmer_mapel_tp WHERE kd = '$xyz'");
				
				
				//insert
				mysqli_query($koneksi, "INSERT INTO kurmer_mapel_tp(kd, tapel, kelas, ".
											"smt, kode, nama, tp_kode, postdate) VALUES ".
											"('$xyz', '$tapelkd', '$kelkd', ".
											"'$smt', '$mkode', '$mnama', '$k', '$today')");
				...
				echo '<td>
				<textarea name="pnama'.$k.'" rows="3" cols="100%" wrap="yes" class="btn-warning">'.$i_pnilai.'</textarea>
				</td>';
