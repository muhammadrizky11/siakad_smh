...
if ($_POST['btnSMP'])
	...
	//jika guru
		...
		if ($e_ket == "Sakit")
			{
			//update kan
			mysqli_query($koneksi, "UPDATE m_guru SET jml_absen_sakit = '$tyuk_subtotal' ".
										"WHERE kode = '$e_kode'");
			}
		...
		//jika ijin
			...
			{
			//update kan
			mysqli_query($koneksi, "UPDATE m_guru SET jml_absen_ijin = '$tyuk_subtotal' ".
										"WHERE kode = '$e_kode'");
			}
		...
		//jika alpha
			...
			{
			//update kan
			mysqli_query($koneksi, "UPDATE m_guru SET jml_absen_alpha = '$tyuk_subtotal' ".
										"WHERE kode = '$e_kode'");
			}
		...
		//detail
		$qyuk = mysqli_query($koneksi, "SELECT * FROM m_guru ".
											"WHERE kode = '$e_kode'");
		$ryuk = mysqli_fetch_assoc($qyuk);
		$yuk_abs_sakit = balikin($ryuk['jml_absen_sakit']);
		$yuk_abs_ijin = balikin($ryuk['jml_absen_ijin']);
		...
		//update kan
		mysqli_query($koneksi, "UPDATE m_guru SET jml_absen = '$yuk_abs' ".
									"WHERE kode = '$e_kode'");
...
<p>
KODE/NIP/NIS : <input name="e_kode" type="text" value="'.$e_kode.'" size="10" class="btn btn-warning" required>, 
