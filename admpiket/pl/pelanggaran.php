...
if ($_POST['btnSMPx2'])
	...
	if ((empty($jnskd)) OR (empty($pelkd)))
		...
		mysqli_query($koneksi, "INSERT INTO siswa_pelanggaran (kd, tapel_nama, kelas_nama, ".
									...
									"piket_kd, piket_kode, piket_nama, piket_jabatan) VALUES ".
									"('$x', '$cc_tapel', '$cc_kelas', ".
									"'$swkd', '$nis', '$cc_nama', ".
									"'$tgl_entry', '$jnskd', '$cc_jeniskode', '$cc_jenisnama', ".
									"'$pelkd', '$cc_pelkode', '$cc_pelnama', ".
									"'$cc_pelpoint', '$cc_pelsanksi', '$today', ".
									"'$kd71_session', '$kode71_session', '$nama71_session', '$jabatan71_session')");
								...
								[Total Point : <b>'.$yuk_subtotal.'</b>].
									...
									if ($count != 0)
										...
										do 
											...
											if ($i_sahya == "false")
												{
												echo '<a href="'.$filenya.'?s=hapus&pkd='.$i_kd.'&swkd='.$swkd.'&nis='.$nis.'" class="btn btn-block btn-danger">HAPUS >></a>
												<br>
												[<font color="red">BELUM DISAHKAN ADMIN</font>]';
												}
