...
$judul = "[GURU MENGAJAR]. Lap. Presensi Guru Mengajar";
$judulku = "$judul";
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if ($_POST['btnEX'])
	...
	<h3>LAPORAN PRESENSI GURU MENGAJAR</h3>
	...
	if ($count != 0)
		...
		do 
			...
			$i_wk_presensi = balikin($data['wk_presensi']);
			$i_wk_catatan = balikin($data['wk_catatan']);
	
	
			//jika hadir
			if ($i_wk_presensi == "H")
				{
				$i_wk_presensi_ket = "Hadir";
				}
			else if ($i_wk_presensi == "I")
				{
				$i_wk_presensi_ket = "Ijin";
				}
			else if ($i_wk_presensi == "S")
				{
				$i_wk_presensi_ket = "Sakit";
				}
			else if ($i_wk_presensi == "A")
				{
				$i_wk_presensi_ket = "Alpha";
				}
			else
				{
				$i_wk_presensi_ket = "";
