...
if ($_POST['btnSMP'])
	...
	for($i=1;$i<=$limit;$i++)
		...
		$abs = "sopredikat";
		$abs1 = "$abs$i";
		$sopredikat = cegah($_POST["$abs1"]);
		
		$abs = "soket";
		$abs1 = "$abs$i";
		$soket = cegah($_POST["$abs1"]);
		...
		if (!empty($swnama))
			{
			//cek
			$qcc = mysqli_query($koneksi, "SELECT kd FROM siswa_raport_sikap ".
												"WHERE kd = '$xyz'");
			...
			if (empty($tcc))
				{
				//query
				mysqli_query($koneksi, "INSERT INTO siswa_raport_sikap(kd, siswa_kode, siswa_nama, ".
											"tapel, kelas, smt, ".
											"spiritual_predikat, spiritual_isi, sosial_predikat, sosial_isi, postdate) VALUES ".
											"('$xyz', '$nisnya', '$swnama', ".
											"'$tapelkd', '$kelkd', '$smt', ".
											"'$sppredikat', '$spket', '$sopredikat', '$soket', '$today')");
				}
				
			else
				{
				//update
				mysqli_query($koneksi, "UPDATE siswa_raport_sikap SET spiritual_predikat = '$sppredikat', ".
											"spiritual_isi = '$spket', ".
											"sosial_predikat = '$sopredikat', ".
											"sosial_isi = '$soket', ".
											"postdate = '$today' ".
											"WHERE kd = '$xyz'");
				}
...
if ($_POST['btnHPS'])
	...
	//hapus
	mysqli_query($koneksi, "DELETE FROM siswa_raport_sikap ".
								"WHERE tapel = '$tapelkd' ".
								"AND kelas = '$kelkd' ".
								"AND smt = '$smt'");	
			    ...
			    if ($i > 5) 
			    	{
				      ...
				      $i_sppred = cegah($sheet['D']);
				      $i_spket = cegah($sheet['E']);
				      $i_sopred = cegah($sheet['F']);
				      $i_soket = cegah($sheet['G']);
						...
						if (!empty($i_swnis))
							...
							mysqli_query($koneksi, "INSERT INTO siswa_raport_sikap(kd, siswa_kode, siswa_nama, ".
														...
														"'$tapelkd', '$kelkd', '$smt', ".
														"'$i_sppred', '$i_spket', '$i_sopred', '$i_soket', '$today')");
							}
...
if ($_POST['btnEX'])
	...
	//nilai
	$qku = mysqli_query($koneksi, "SELECT * FROM siswa_raport_sikap ".
										"WHERE tapel = '$tapelkd' ".
										"AND kelas = '$kelkd' ".
										"AND smt = '$smt'");
	...
	<thead>
		<tr valign="top" bgcolor="'.$warnaheader.'">
			...
			<td colspan="2">
				<p align="center"><b>SIKAP SPIRITUAL</b></p>
			</td>
			<td colspan="2">
				<p align="center"><b>SIKAP SOSIAL</b></p>
			</td>
		</tr>
		<tr valign="top" bgcolor="'.$warnaheader.'">
			<td width="5">
	...
	<tbody>';
		...
		do
			...
			$rku = mysqli_fetch_assoc($qku);
			$i_sppredikat = balikin($rku['spiritual_predikat']);
			$i_spket = balikin($rku['spiritual_isi']);
			$i_sopredikat = balikin($rku['sosial_predikat']);
			$i_soket = balikin($rku['sosial_isi']);
			...
			echo '<td>
				...
				<td>'.$i_sppredikat.'</td>
				<td align="left">'.$i_spket.'</td>
				<td>'.$i_sopredikat.'</td>
				<td align="left">'.$i_soket.'</td>
			...
			<tbody>';
				...
				do
					...
					echo '<td>
						...
						<td>
							<select name="sppredikat'.$nomer.'" class="btn btn-warning">
							<option value="'.$i_sppredikat.'" selected>'.$i_sppredikat.'</option>
							<option value="Baik">Baik</option>
							<option value="Cukup">Cukup</option>
							<option value="Kurang Baik">Kurang Baik</option>
						</td>
						<td>
							<textarea cols="20" name="spket'.$nomer.'" rows="5" wrap="yes" class="btn-warning">'.$i_spket.'</textarea>
						</td>
						<td>
							<select name="sopredikat'.$nomer.'" class="btn btn-warning">
							<option value="'.$i_sopredikat.'" selected>'.$i_sopredikat.'</option>
							<option value="Baik">Baik</option>
							<option value="Cukup">Cukup</option>
							<option value="Kurang Baik">Kurang Baik</option>
						</td>
						<td>
							<textarea cols="20" name="soket'.$nomer.'" rows="5" wrap="yes" class="btn-warning">'.$i_soket.'</textarea>
						</td>
