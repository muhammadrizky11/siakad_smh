...
//siswa ne
$qsiw = mysqli_query($koneksi, "SELECT * FROM m_siswa ".
			"WHERE kd = '$swkd'");
...
if (empty($_REQUEST['ubln']))
	...
	for ($i=1;$i<=$tkhir;$i++)
		...
		//nilai data
		$qdtf = mysqli_query($koneksi, "SELECT * FROM siswa_absensi ".
		...
		//absensinya
		$qbein = mysqli_query($koneksi, "SELECT * FROM m_absensi ".
								"WHERE kd = '$dtf_abskd'");
		$rbein = mysqli_fetch_assoc($qbein);
